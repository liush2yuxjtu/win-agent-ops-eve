...
class Store:
    ...
    def finish_model_run(self, run_id, report=None, error=None):
        with self.connect() as c:
            ...
            analysis=dict(report['diagnosis'],engine='eve-cpa-1',family='model',sourceRevision=run['source_revision'],runId=run_id,sessionId=report['sessionId'],variant=report.get('variant','baseline'))
