...
with sync_playwright() as p:
    ...
    original_issue = page.locator('.issue-item.active').get_attribute('data-issue-id')
    assert page.get_by_role('button', name='a  APPLY SELECTED FIX', exact=True).count() == 0
    page.keyboard.press('l')
    assert page.locator('.raw-log[open]').count() == 1
    page.keyboard.press('l')
    assert page.locator('.raw-log[open]').count() == 0
    page.keyboard.press('j')
    assert page.locator('.issue-item.active').get_attribute('data-issue-id') != original_issue
    ...
    assert page.get_by_role('button', name='a  APPLY SELECTED FIX', exact=True).count() == 1
    page.keyboard.press(']')
    assert page.locator('.route.selected .route-rank').inner_text() == '02'
    page.keyboard.press('[')
    assert page.locator('.route.selected .route-rank').inner_text() == '01'
    ...
    result = {
        ...
        'checks': [
            ...
            '原始长日志展开/收起', '唯一 AskUserQuestion 审批门', '弹窗内快捷键不串题',
            'Agent steps 动态展示', '问题已复现/问题已解决/输入反馈',
            '5 条路线与 Blast Radius', 'Probe 完成后才能应用路线',
