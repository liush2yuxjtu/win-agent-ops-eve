...
def evidence_packet(store, key, question='解释这个问题，比较根因并给出最多五个互补方案。'):
    ...
    evidence[0]['kind']='stored-monitor-result-not-fresh-probe'
    evidence[0]['content']['observationSemantics']='status/detail 是监控保存的结果；last_seen 是源快照心跳，不是本次实际登录时间。认证失败可能缓存。本证据包未提供每个检查的实际执行时间，不能据此确认用户此刻仍无法登录，也不能把重复保存说成重复实测。'
