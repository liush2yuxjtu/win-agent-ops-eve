...
工作台运行在 `http://127.0.0.1:8766/`，当前采用已批准的三步 Triage Funnel 真实界面。先把原始长日志整理成全部问题并分到 `P0/P1/P2/P3`，再为一个问题打开唯一 `AskUserQuestion` 探针审批门，最后比较 5 条路线并由人记录应用或暂缓。Eve 根据真实监控、历史检索、此前判断及人工反馈生成独立处理路线。旧技术诊断保留，不能冒充新版领导简报。每次生成新版本，不把采纳当作业务已执行。真实应用录制与改动说明见 `docs/REAL-APP-RECORDING.md`；设计契约见 `docs/PRODUCT-DEMO-DESIGN.md`。
